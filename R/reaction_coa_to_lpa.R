...
#'
#' @export
coa_to_lpa <- function(CoAs, template = NA, constraints = c(""), negate = c(FALSE)) {
  
  # make hard coded template for reaction
  if(is.na(template)) {
    template <- list(reaction_name = "",
                     reaction_formula = "M_glyc3p + M_fataccoa <=> M_coa + M_alpa_pl",
  ...
  LPAs <- stringr::str_replace_all(CoAs, "CoA", "PA")
