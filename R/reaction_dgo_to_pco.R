...
#'
#' @export
dgo_to_pco <- function(DGOs, template = NA, constraints = c(""), negate = c(FALSE)) {
  ...
  PCOs <- stringr::str_replace_all(DGOs, "DG", "PC")
  PCOs <- stringr::str_replace(PCOs, "/0:0\\)$", "\\)")
  ...
  # return results
  return(list(unique(PCOs), reaction_df))
