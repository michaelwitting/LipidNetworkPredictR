...
#'
#' @export
pa_to_dg <- function(PAs, template = NA, constraints = c(""), negate = c(FALSE)) {
  ...
  DGs <- stringr::str_replace_all(PAs, "PA", "DG")
  DGs <- stringr::str_replace_all(DGs, "\\)$", "/0:0\\)")
  ...
  # return results
  return(list(unique(DGs), reaction_df))
