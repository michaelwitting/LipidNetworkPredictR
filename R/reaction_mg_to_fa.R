...
#'
#' @export
sn1mg_to_fa <- function(sn1MGs, template = NA, constraints = c(""), negate = c(FALSE)) {
  
  # make hard coded template for reaction
  if(is.na(template)) {
    template <- list(reaction_name = "",
                     reaction_formula = "M_h2o + M_magol <=> M_glyc + M_h + M_fatacid",
  ...
  # "sn2 loss"
  FAs <- unlist(lapply(sn1MGs, function(sn1mg) {
    fatty_acyls <- lipidomicsUtils::isolate_fatty_acyls(sn1mg)
...
#'
#' @export
sn2mg_to_fa <- function(sn2MGs, template = NA, constraints = c(""), negate = c(FALSE)) {
  ...
  # "sn2 loss"
  FAs <- unlist(lapply(sn2MGs, function(sn2mg) {
