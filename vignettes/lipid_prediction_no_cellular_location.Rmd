...
Fatty acids used in glycero- and glycerophospholipids are different from the ones used in sphingolipid biosynthesis. The pools are only partially overlapping between the two. Therefore two different files for the two different pools are supplied. The following lines read the fatty acids into a data frame and isolates the shorthand notation of the fatty acids, which are further used for downstream processing.
...
Diacylglycerols (DGs) are produced from PAs by dephosphorylation. DGs are important intermediates for biosynthesis of glycerophospholipids or triacylglycerols (DGs). A prominent enzyme catalyzing this reactions is LPIN-1. Plasma membrane or ER WBGene00008749 or WBGene00018756 or WBGene00020895 or WBGene00010425
...
DGs can be phosphorylated to create PAs, which is the reversal of the reactoin above.
...
DGs are degraded by the activity of different lipases yieling mono acylglycerols. In the case of this reaction the fatty acid at sn1 position is removed yieling an sn2-MG. Sn2-MGs can be converted to sn1-MGs, which in turn can be re-acylated at a later stage.
...
Both, sn1- and sn2-MGs, can degraded to free fatty acids and glycerol, which can be used for energy production.
...
Similar to the degradation, all MGs can be reacylated to yields DGs again. Additionally, 1-Acyl-MGs can be also phosphorylated to yield LPAs.
...
The glycerophospholipids phosphatidylcholine, -ethanolamine and serine are synthesized from DGs by addition of headgroups. Additionally they can be interconverted
...
Some etherlipids... mostly PE, small amounts in PCs This reaction is constrained to 16:0, 18:0, 20:0!
...
if(all(results[[2]] %in% sn2LPCs)) {
  ...
  lipid_info <- parseLipidNames(NAPEs)
...
if(all(results[[2]] %in% sn2LPCs)) {
  ...
  lipid_info <- parseLipidNames(NAPEOs)
...
if(all(results[[2]] %in% sn2LPCs)) {
  ...
  lipid_info <- parseLipidNames(NAPEPs)
...
if(all(results[[2]] %in% PAs)) {
  ...
  lipid_info <- parseLipidNames(NAEs)
...
if(all(results[[2]] %in% DGs)) {
  ...
  lipid_info <- parseLipidNames(PNAEs)
...
if(all(results[[2]] %in% FAs_GPGL)) {
  ...
  lipid_info <- parseLipidNames(LNAPEs)
...
if(all(results[[2]] %in% FAs_GPGL)) {
  ...
  lipid_info <- parseLipidNames(GPNAEs)
