...
*Caenorhabditis elegans* (*C. elegans*) lipids performed by Witting 
et al. (in preparation). Input are fatty acids that were shown to be present 
in *C. elegans* by different groups. A consenus list was generated based on 
fatty acids from WormJam and detected in *C. elegans*. A data frame of these 
fatty acids is supplied with this package.
...
In order to store all the results a data frame for the reactions is created. 
Lipids abbreviations are just appended.
...
The following abbreviations are used (in alphabetical order):
 ...
 - `ACDHAP`: acyl-dihydroxyacetonephosphate,
 ...
 - `CL`: Diacylglycerophosphoglycerophosphodiradylglycerols (cardiolipin) [GP1201],
...
Fatty acids used in glycero- and glycerophospholipids are different from the 
ones used in sphingolipid biosynthesis. The pools are only partially 
overlapping between the two. Therefore, two different files for the two 
different pools are supplied. The following lines read the fatty acids into a 
data frame and isolates the shorthand notation of the fatty acids, which are 
further used for downstream processing.
...
`list`, e.g. gene associations etc. This information can be obtained from 
WormJam.
...
returns a list with multiple elements. The second element contains the new 
non-generic reactions, whereas the first contain the shorthand notation 
of all newly generated lipids. Since the shorthand notations are required 
further downstream, they are additionally stored object with the name of the 
respective lipid class. The lines below generate acyl-CoA for fatty acids 
found to be present in glycero- and glycerophospholipids.

```{r, message=FALSE, warning=FALSE}
## convert fatty acids to acyl CoA --------------------------------------------
results <- create_reactions(substrates = list(FA = FA_GPGL), 
    template = template, reaction = "RHEA:15421")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["CoA"]])

## get CoA for further reactions (glycero- and glycerophospholipids)
CoA_GPGL <- results[[1]][["CoA"]]
...
### Biosynthesis of CoA for Ceramide biosynthesis

<!-- ![Test](images/FA_to_CoA.png){width="500px"} -->

Likewise, the fatty acyls for the synthesis of sphingolipids are required as 
acyl-CoA.

```{r, message=FALSE, warning=FALSE}
## convert fatty acids to acyl CoA --------------------------------------------
results <- create_reactions(substrates = list(FA = FA_SPH), template = template,
    reaction = "RHEA:15421")
results[[2]][1:10, "ReactionFormula"]
...
Diacylgylcerols (DG) are an important intermediate for the biosynthesis of 
different lipids. The represent a branching point between triacylglycerols and
glycerophospholipids. They are synthesized from lysosphosphaditic and 
phosphaditic acid by successive acylation of glycerol 3-phosphate. The first 
block of reactions handles the generation of PA and DG as well as their 
metabolism via monoacylglycerols to fatty acids.
...
in the ER and the mitochondria. In the ER the reaction is catalyzed by acl-5 
(WBGene00011106) or acl-6 (WBGene00017261), while in the mitochondria acl-4 
(WBGene00018657) is used. No subcellular specificity is currently used, 
therefore the assignments are merged together to yield a single reaction.
...
## biosynthesis of LPA --------------------------------------------------------
results <- create_reactions(substrates = list(CoA = CoA_GPGL), template = template,
    reaction = "RHEA:15325")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["LPA"]])
...
In the next step a second acyl group is transferred to the LPA to produce PA. 
This reaction is catalyzed by acl-9 (WBGene00022646) or acl-8 (WBGene00020264) 
or mboa-3 (WBGene00016934) or acl-1 (WBGene00010339) or acl-2 
(WBGene00011543) in the ER. For the mitochondrial counterpart no genes are 
known so far. acl-3 (WBGene00006491) might be a good candidate since it is 
predicted to locate in the mitochondria.
...
## biosynthesis of PA ---------------------------------------------------------
results <- create_reactions(substrates = list(LPA = LPA, CoA = CoA_GPGL), 
    template = template, reaction = "RHEA:19709")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["PA"]])
...
WBGene00020895 or WBGene00010425
...
```{r, message=FALSE, warning=FALSE}
results <- create_reactions(substrates = list(PA = PA), template = template,
    reaction = "pa_to_dg")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["DG"]])
...
results <- create_reactions(substrates = list(DG = DG), template = template,
    reaction = "RHEA:10272")
results[[2]][1:10, "ReactionFormula"]
...
```{r, message=FALSE, warning=FALSE}
results <-  create_reactions(substrates = list(DG = DG), template = template,
    reaction = "dg_to_sn2mg")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["sn2MG"]])
...
```{r, message=FALSE, warning=FALSE}
results <-  create_reactions(substrates = list(sn2MG = sn2MG), template = template,
...
Both, sn1- and sn2-MG, can degraded to free fatty acids and glycerol, which 
...
```{r, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(sn1MG = sn1MG), template = template,
    reaction = "sn1mg_to_fa")
results[[2]][1:10, "ReactionFormula"]
...
Additionally, 1-Acyl-MG can be also phosphorylated to yield LPA.
...
```{r, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(sn1MG = sn1MG, CoA = CoA_GPGL), 
    template = template, reaction = "sn1mg_to_dg")
results[[2]][1:10, "ReactionFormula"]
...
```{r, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(sn2MG = sn2MG, CoA = CoA_GPGL), 
    template = template, reaction = "sn2mg_to_dg")
results[[2]][1:10, "ReactionFormula"]
...
```{r, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(sn1MG = sn1MG), 
    template = template, reaction = "sn1mg_to_lpa")
results[[2]][1:10, "ReactionFormula"]
...
Triacylglycerols (TG) are storage lipids in *C. elegans* and other organisms. 
They are stored in lipid droplets. In *C. elegans* they are also found in yolk 
...
```{r dg_to_tg_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r dg_to_tg, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(DG = DG, CoA = CoA_GPGL), 
    template = template, reaction = "RHEA:10868")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector 
lipid_abbrev <- c(lipid_abbrev, results[[1]][["TG"]])
...
```{r tg_to_dg_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r tg_to_dg, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(TG = TG), template = template,
    reaction = "tg_to_dg")
results[[2]][1:10, "ReactionFormula"]
...
```{r dg_to_pc_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(
    reaction_name = "CDP-choline 1,2-diacyl-sn-glycerol phosphocholine transferase",
...
```{r dg_to_pc, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(DG = DG), template = template,
    reaction = "RHEA:32939")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["PC"]])
...
```{r dg_to_pe_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r dg_to_pe, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(DG = DG), template = template,
    reaction = "RHEA:32943")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["PE"]])
...
```{r pc_to_ps_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pc_to_ps, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PC = PC), template = template,
    reaction = "RHEA:45088")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["PS"]])
...
```{r pe_to_ps_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pe_to_ps, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PE = PE), template = template,
    reaction = "RHEA:27606")
results[[2]][1:10, "ReactionFormula"]
...
```{r ps_to_pe_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r ps_to_pe, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PS = PS), template = template,
    reaction = "ps_to_pe")
results[[2]][1:10, "ReactionFormula"]
...
```{r pc_to_sn2lpc_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pc_to_sn2lpc, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PC = PC), template = template,
    reaction = "RHEA:18689")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["sn2LPC"]])
...
```{r pc_to_sn1lpc_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pc_to_sn1lpc, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PC = PC), template = template,
    reaction = "RHEA:15801")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["sn1LPC"]])
...
```{r pc_to_pa_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pc_to_pa, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PC = PC), template = template,
    reaction = "RHEA:14445")
results[[2]][1:10, "ReactionFormula"]
...
```{r sn1lpc_to_fa_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r sn1lpc_to_fa, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(sn1LPC = sn1LPC), 
...
```{r sn2lpc_to_fa_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r sn2lpc_to_fa, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(sn2LPC = sn2LPC), 
...
```{r sn1lpc_to_pc_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r sn1lpc_to_pc, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(sn1LPC = sn1LPC, CoA = CoA_GPGL),
    template = template, reaction = "RHEA:12937", 
    constraints = c("", "OH"), negate = c(FALSE, TRUE))
results[[2]][1:10, "ReactionFormula"]
...
```{r pe_to_sn2lpe_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pe_to_sn2lpe, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PE = PE), template = template,
    reaction = "RHEA:44408")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["sn2LPE"]])
...
```{r pe_to_sn1lpe_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pe_to_sn1lpe, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PE = PE), template = template,
    reaction = "pe_to_sn1lpe")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["sn1LPE"]])
...
```{r pe_to_pa_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pe_to_pa, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PE = PE), template = template,
    reaction = "pe_to_pa")
results[[2]][1:10, "ReactionFormula"]
...
```{r sn1lpe_to_fa_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r sn1lpe_to_fa, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(sn1LPE = sn1LPE), 
...
```{r sn2lpe_to_fa, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(sn2LPE = sn2LPE), 
...
```{r sn1lpe_to_pe_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r sn1lpe_to_pe, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(sn1LPE = sn1LPE, CoA = CoA_GPGL), 
    template = template, reaction = "RHEA:32995", constraints = c("", "OH"),
    negate = c(FALSE, TRUE))
results[[2]][1:10, "ReactionFormula"]
...
```{r pa_to_cdpdg_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pa_to_cdpdg, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PA = PA), template = template,
    reaction = "pa_to_cdpdg")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["CDPDG"]])
...
```{r cdpdg_to_pi_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r cdpdg_to_pi, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(CDPDG = CDPDG), template = template,
    reaction = "RHEA:11580")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["PI"]])
...
```{r cdpdg_to_pgp_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r cdpdg_to_pgp, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(CDPDG = CDPDG), template = template,
    reaction = "RHEA:12593")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["PGP"]])
...
```{r pgp_to_pg_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pgp_to_pg, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PGP = PGP), template = template,
    reaction = "pgp_to_pg")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["PG"]])
...
```{r pg_to_cl_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "Phosphatidylglycerophosphatase",
...
```{r pg_to_cl, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PG = PG, CDPDG = CDPDG), 
    template = template, reaction = "pg_to_cl",
...
## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["CL"]])
...
```{r coa_to_fatoh_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "Fatty acyl-CoA reductase",
    reaction_formula = "M_fataccoa + 2 M_nadph + 2 M_h <=> M_alkylR1oh + 2 M_nadp + M_coa",
    reactin_RHEA = "RHEA:52716",
    reaction_isReversible = "FALSE",
    reaction_geneAssociation = "WBGene00022200",
    reaction_pathway = "Biosynthesis of Etherlipids")
```

```{r coa_to_fatoh, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(CoA = CoA_GPGL), 
    template = template, reaction = "RHEA:52716", 
    constraints = c("16:0)|18:0)|20:0)"), negate = c(FALSE))
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["FATOH"]])
...
```{r coa_to_acdhap_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r coa_to_acdhap, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(CoA = CoA_GPGL), 
    template = template, reaction = "coa_to_acdhap")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev,  results[[1]][["DHAP"]])

## get ACDHAP for further reactions
ACDHAP <- results[[1]][["DHAP"]]

## add new reactions and add lipids to data frame
lipid_reactions <- rbind.data.frame(lipid_reactions, results[[2]])
lipid_info <- rgoslin::parseLipidNames(ACDHAP)
lipid_df <- dplyr::bind_rows(lipid_df, lipid_info)
```

### Generation of Alkyl-DHAP

```{r acdhap_to_alkyldhap_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r acdhap_to_alkyldhap, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(ACDHAP = ACDHAP, FATOH = FATOH), 
    template = template, reaction = "RHEA:36171") 
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["ALKYLDHAP"]])
...
```{r alkyldhap_to_lpao_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "1-acylglycerone phosphate reductase",
    reaction_formula = "M_h + M_nadph + M_akdhap <=> M_alpa_pl + M_nadp",
...
```{r alkyldhap_to_lpao, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(ALKYLDHAP = ALKYLDHAP),
    template = template, reaction = "RHEA:36175")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["LPAO"]])
...
```{r lpao_to_pao_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
##template <- list(reaction_name = "2-O-Acyl transferase",
##    reaction_formula = "M_alpa_pl + M_fataccoa <=> M_pa_pl + M_coa",
##    reaction_RHEA = "RHEA:19709",
##    reaction_isReversible = "FALSE",
##    reaction_geneAssociation = "WBGene00022646 or WBGene00020264 or WBGene00016934 or WBGene00010339 or WBGene00011543",
##    reaction_pathway = "Biosynthesis of Etherlipids")
...
```{r lpao_to_pao, message=FALSE, warning=FALSE, eval=FALSE}
##results <-  create_reactions(substrates = list(LPAO = LPAO, CoA = CoA_GPGL), 
##    template = template, reaction = "RHEA:19709",
##    constraints = c("", "OH"), negate = c(FALSE, TRUE))
##results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
##lipid_abbrev <- c(lipid_abbrev, results[[1]][["PAO"]])
...
```{r pao_to_dgo_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pao_to_dgo, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PAO = PAO), template = template,
    reaction = "pao_to_dgo")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["DGO"]])
...
```{r dgo_to_peo_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r dgo_to_peo, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(DGO = DGO), template = template,
    reaction = "RHEA:36187")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["PEO"]])
...
```{r peo_to_pep_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r peo_to_pep, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PEO = PEO), template = template,
    reaction = "peo_to_pep")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["PEP"]])
...
```{r dgo_to_pco_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r dgo_to_pco, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(DGO = DGO), template = template,
    reaction = "RHEA:36179")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["PCO"]])
...
```{r peo_to_lpeo_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r peo_to_lpeo, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PEO = PEO), template = template,
    reaction = "peo_to_lpeo")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["LPEO"]])
...
```{r lpeo_to_peo_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r lpeo_to_peo, message=FALSE, warning=FALSE, eval=FALSE}
# biosynthesis of PEO ----------------------------------------------------------
results <-  create_reactions(substrates = list(LPEO = LPEO, CoA = CoA_GPGL), 
...
```{r pep_to_lpep_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pep_to_lpep, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PEP = PEP), template = template,
    reaction = "pep_to_lpep")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["LPEP"]])
...
```{r lpep_to_pep_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r lpep_to_pep, message=FALSE, warning=FALSE, eval=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(LPEP = LPEP, CoA = CoA_GPGL), 
...
```{r pco_to_lpco_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r pco_to_lpco, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PCO = PCO), template = template,
    reaction = "pco_to_lpco")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["LPCO"]])
...
```{r sphinga_to_dhcer, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r sphinga_to_dhcer_template, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(CoA = CoA_SPH), template = template,
    reaction = "sphinga_to_dhcer")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["DHCER"]])
...
```{r dhcer_to_dhsm_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r dhcer_to_dhsm, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(DHCER = DHCER), 
    template = template, reaction = "RHEA:18765")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["DHSM"]])
...
```{r dhsm_to_dhcer_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r dhsm_to_dhcer, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(DHSM = DHSM), template = template,
    reaction = "RHEA:19253")
results[[2]][1:10, "ReactionFormula"]
...
```{r dhcer_to_cer_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r dhcer_to_cer, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(DHCER = DHCER), template = template,
    reaction = "dhcer_to_cer")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["CER"]])
...
```{r cer_to_sm_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r cer_to_sm, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(CER = CER), template = template,
    reaction = "RHEA:18765")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["SM"]])
...
```{r sm_to_cer_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r sm_to_cer, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(SM = SM), template = template,
    reaction = "sm_to_cer")
results[[2]][1:10, "ReactionFormula"]
...
```{r cer_to_c1p_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "Ceramide kinase",
    reaction_formula = "M_atp + M_c17isocrm <=> M_h + M_adp + M_c17isocrmp",
    reaction_RHEA = reaction,
    reaction_isReversible = "FALSE",
    reaction_geneAssociation = "WBGene00020398",
    reaction_pathway = "Biosynthesis and degradation of sphingolipids")
```

```{r cer_to_c1p, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(CER = CER), template = template,
    reaction = "RHEA:17929")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["C1P"]])
...
```{r c1p_to_cer_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r c1p_to_cer, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(C1P = C1P), template = template,
    reaction = "c1p_to_cer")
results[[2]][1:10, "ReactionFormula"]
...
```{r cer_to_glccer_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
...
```{r cer_to_glccer, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(CER = CER), template = template,
    reaction = "RHEA:12088")
results[[2]][1:10, "ReactionFormula"]

## add new lipids to lipid vector
lipid_abbrev <- c(lipid_abbrev, results[[1]][["GLCCER"]])
...
N-acyl-phosphatidylethanolamines are produced by the transfer of an acyl group 
from PC to the nitrogen of PE. In *C. elegans* this reaction is so far not known. 
It is unknown if the sn1 or sn2 position is transferred. Since *C. elegans* 
contains about equal amounts of PE, PE-O and PE-P it might be possible that 
the acyl group is transferred to all of them producing NAPE, NAPE-O and NAPE-P.

transfer of Sn1 fatty acyl to PE

```{r pe_to_nape_sn1_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r pe_to_nape_sn1, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PE = PE, PC = PC), 
    template = template, reaction = "pe_to_nape_sn1")
results[[2]][1:10, "ReactionFormula"]

## check if produced sn2LPC are in previous list
if (all(results[[1]][["LPC"]] %in% sn2LPC)) {
...
```{r peo_to_napeo_sn1_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r peo_to_napeo_sn1, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PEO = PEO, PC = PC), 
...
```{r pep_to_napep_sn1_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r pep_to_napep_sn1, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PEP = PEP, PC = PC), 
...
```{r pe_to_nape_sn2_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r pe_to_nape_sn2, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PE = PE, PC = PC), 
    template = template, reaction = "pe_to_nape_sn2")
results[[2]][1:10, "ReactionFormula"]

## check if produced sn1LPC and NAPE are in previous list
if (all(results[[1]][["LPC"]] %in% sn1LPC) & all(results[[1]][["NAPE"]] %in% NAPE)) {
...
```{r peo_to_napeo_sn2_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r pep_to_napep_sn2_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r pep_to_napep_sn2, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(PEP = PEP, PC = PC), template = template,
...
```{r nape_to_nae_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r nape_to_nae, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(NAPE = NAPE), template = template,
...
```{r napeo_to_nae_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r napeo_to_nae, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(NAPEO = NAPEO), 
...
```{r nape_to_pnae_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r nape_to_pnae, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(NAPE = NAPE), template = template,
...
```{r nape_to_lnape_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r nape_to_lnape, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(NAPE = NAPE), template = template,
...
```{r lnape_to_gpnae_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r lnape_to_gpnae, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(LNAPE = LNAPE), template = template,
    reaction = "RHEA:45420")
results[[2]][1:10, "ReactionFormula"]
...
```{r nae_to_fa_template, message=FALSE, warning=FALSE, eval=FALSE}
## template for reaction -------------------------------------------------------
template <- list(reaction_name = "",
...
```{r nae_to_fa, message=FALSE, warning=FALSE, eval=FALSE}
results <-  create_reactions(substrates = list(NAE = NAE), template = template,
    reaction = "nae_to_fa")
results[[2]][1:10, "ReactionFormula"]
